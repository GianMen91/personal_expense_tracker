...
class ExpensesListBloc extends Bloc<ExpensesListEvent, ExpensesListState> {
  ...
  ExpensesListBloc(this.dbHelper) : super(ExpensesListState(expenses: [], isLoading: true)) {
    on<LoadExpense>(_onLoadExpense);
    on<AddExpense>(_onAddExpense);
    on<DeleteExpense>(_onDeleteExpense);
  }

  Future<void> _onLoadExpense(LoadExpense event, Emitter<ExpensesListState> emit) async {
  ...
  Future<void> _onAddExpense(AddExpense event, Emitter<ExpensesListState> emit) async {
    try {
  ...
  Future<void> _onDeleteExpense(DeleteExpense event, Emitter<ExpensesListState> emit) async {
    try {
  ...
  ExpensesListState _updateStateWithDerivedData(ExpensesListState currentState) {
  ...
  ExpenseData _calculateExpenseData(List<Expense> expenses) {
    ...
    return ExpenseData(groupedExpenses: groupedExpenses, monthlyTotal: monthlyTotal, expenses: expenses);
  }
  ...
  DateTime _parseDate(String dateStr) {
    ...
    return DateFormat('dd MMM yyyy').parse(dateStr); // Corrected format
  }
  ...
  String _formatDate(DateTime date) {
    ...
    return DateFormat('dd MMM yyyy').format(date); // Corrected format
  }
